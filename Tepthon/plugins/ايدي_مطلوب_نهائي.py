...
################# Dev ZilZal #################
    ...
    likers_key = f"LIKERS_{user_id}"
    likers = gvarstatus(likers_key)
    likers_list = likers.split() if likers else []
    likes_count = len(likers_list)

    if gvarstatus("ZID_TEMPLATE") is None:
        caption += f"<b>{ZEDM}المعجبين   ⤎</b>  {likes_count} 🩷\n"
...
@zedub.zed_cmd(
    pattern="لايك$",
    command=("لايك", plugin_category),
    info={
        "header": "لإضافة لايك للمستخدم",
        "الاستـخـدام": "رد على رسالة .ايدي فقط لإضافة لايك وتسجيلك كمعجب",
    },
)
async def like_user(event):
    if not event.reply_to_msg_id:
        return await edit_or_reply(event, "**⎉╎يجب الرد على رسالة تحتوي على .ايدي لإضافة لايك.**")

    reply_msg = await event.get_reply_message()
    import re
    match = re.search(r"<code>(\d{6,})</code>", reply_msg.raw_text)
    if not match:
        return await edit_or_reply(event, "**⎉╎لم أستطع تحديد الشخص المستهدف من الرسالة.**")

    target_id = int(match.group(1))

    current_likes = gvarstatus(f"LIKES_{target_id}")
    current_likes = int(current_likes) if current_likes else 0
    current_likes += 1
    addgvar(f"LIKES_{target_id}", str(current_likes))

    # حفظ المعجبين
    likers_key = f"LIKERS_{target_id}"
    current_likers = gvarstatus(likers_key)
    if current_likers:
        if str(event.sender_id) in current_likers.split():
            return await edit_or_reply(event, "**⎉╎أنت بالفعل من ضمن المعجبين.**")
        updated_likers = current_likers + f" {event.sender_id}"
    else:
        updated_likers = str(event.sender_id)
    addgvar(likers_key, updated_likers)

    await edit_or_reply(event, f"**⎉╎تم إعطاء لايك للمستخدم بنجاح 🩷\n⎉╎العدد الآن:** {current_likes}")
